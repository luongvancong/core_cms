    ...
    <form class="form-horizontal bucket-form" method="post" action enctype="multipart/form-data">
        ...
        <div class="form-group">
            <div class="col-sm-6 col-sm-offset-3 text-center">
                <img src="{{ $product->getImage('md_') }}" height="100">
            </div>
        </div>
        @endif
        ...
        <div class="form-group {{ hasValidator('image') }}">
            ...
            <div class="col-sm-6 text-center">
                ...
                <input type="text" name="image_alt" class="form-control" value="{{ $product->getImageAlt() }}" placeholder="Alt">
                {!! alertError('image') !!}
            </div>
        </div>

        @if($product->hasImageHomePage())
        <div class="form-group">
            <div class="col-sm-6 col-sm-offset-3 text-center">
                <img src="{{ $product->getImageHomePage('md_') }}" height="100">
            </div>
        </div>
        @endif

        <div class="form-group">
            <label for="email" class="col-sm-3 control-label">Ảnh hiển thị trên trang chủ</label>
            <div class="col-sm-6 text-center">
                <input type="file" name="image_homepage" class="form-control">
                <input type="text" name="image_homepage_alt" class="form-control" value="{{ $product->getImageHomePageAlt() }}" placeholder="Alt">
            </div>
        </div>
        ...
        <div class="form-group {{ hasValidator('type') }}">
            ...
            <div class="col-sm-6">
                <select name="type" class="form-control">
                    ...
                    <option value="{{ $key }}" {{ $key == $product->getType() ? 'selected="selected"' : '' }}>{{ $value }}</option>
                    @endforeach
                </select>
                {!! alertError('type') !!}
            </div>
        ...
        <div class="form-group">
            <label for="email" class="col-sm-3 control-label">
                Khách hàng <br/>
                (<small> Điền ô này nếu là sản phẩm tư vấn thiết kế </small>)
            </label>
            <div class="col-sm-6 text-center">
                <input type="text" class="form-control" value="{{ Request::old('customer_name', $product->customer_name) }}" name="customer_name">
            </div>
        </div>
        ...
        <div class="form-group {{ hasValidator('code') }}">
            ...
            <div class="col-sm-6 text-center">
                <input type="text" class="form-control" value="{{ Request::old('name', $product->getCode()) }}" name="code">
                {!! alertError('code') !!}
            </div>
        </div>
