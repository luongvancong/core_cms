...
@section('main-content')
    <section class="panel">
        <header class="panel-heading">
            <h4>
                Ảnh chi tiết chuyến xe
            </h4>
            <h5>{{ $trip->startPlace()->first()->getName() }}-{{ $trip->endPlace()->first()->getName() }}</h5>
        </header>
        <div class="panel-body">
            <form class="form-horizontal bucket-form" method="post" action enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <a href="{{ route('admin.trip.images', $trip->getId()) }}" class="btn btn-link">{{ trans('form.btn.back') }}</a>
                    </div>
                </div>
            </form>
        </div>
