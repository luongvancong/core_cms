 ...
 */
add_action( 'widgets_init', array( 'Asalah_InstagramSlider', 'register_widget' ) );

/**
 * Asalah_InstagramSlider Class
 */
class Asalah_InstagramSlider extends WP_Widget {
	...
	public function __construct() {
		...
		parent::__construct( 'jr_insta_slider', __( 'Writing - Instagram Slider', 'jrinstaslider' ), array(
	...
	public function public_enqueue() {
		...
		wp_enqueue_style( 'instag-slider', get_template_directory_uri() . '/inc/widgets/assets/css/instag-slider.css' , array(), self::VERSION );

		wp_enqueue_script( 'jquery-pllexi-slider', get_template_directory_uri() . '/inc/widgets/assets/js/jquery.flexslider-min.js' , array( 'jquery' ), '2.2', false );
	}
	...
	public function admin_enqueue( $hook ) {
		...
		wp_enqueue_style( 'jr-insta-admin-styles', get_template_directory_uri() . '/inc/widgets/assets/css/jr-insta-admin.css' , array(), self::VERSION );

		wp_enqueue_script( 'jr-insta-admin-script', get_template_directory_uri() . '/inc/widgets/assets/js/jr-insta-admin.js' , array( 'jquery' ), self::VERSION, true );
			...
			if ( $instagram_images->have_posts() ) {
				...
				while ( $instagram_images->have_posts() ) : $instagram_images->the_post();
					...
					if ( 'image_url' == $images_link ) {
						...
						$template_args['link_to'] = 'https://instagram.com/' . $username;
				...
				if ( is_array( $args['description'] ) && count( $args['description'] ) >= 1 ) {
					...
					$output .= "<div class='jr-insta-datacontainer'>\n";
						...
						if ( in_array( 'username', $args['description'] ) && $username ) {
							$output .= "<span class='jr-insta-username'>by <a rel='nofollow' href='https://instagram.com/{$username}' target='_blank'>{$username}</a></span>\n";
						}

						if ( $caption != '' && in_array( 'caption', $args['description'] ) ) {
							$caption   = preg_replace( '/@([a-z0-9_]+)/i', '&nbsp;<a href="https://instagram.com/$1" rel="nofollow" target="_blank">@$1</a>&nbsp;', $caption );
				...
				if ( is_array( $args['description'] ) && count( $args['description'] ) >= 1 ) {
					...
					$output .= "<div class='jr-insta-wrap'>\n";
						...
						$output .= "<div class='jr-insta-datacontainer'>\n";
							...
							if ( in_array( 'username', $args['description'] ) && $username ) {
								$output .= "<span class='jr-insta-username'>by <a rel='nofollow' target='_blank' href='https://instagram.com/{$username}'>{$username}</a></span>\n";
							}
	...
	* Trigger refresh for new data
	 ...
	 private function trigger_refresh_data( $instaData, $old_args, $new_args ) {
		 ...
		 if ( is_array( $old_args ) && is_array( $new_args ) && (array_diff_assoc( $old_args, $new_args ) !== array_diff_assoc( $new_args, $old_args )) ) {
			 $trigger = 1;
		 }
	...
	public function insta_attachment_fields( $form_fields, $post ) {
		...
		if ( !empty( $instagram_username ) ) {
			...
			$form_fields["jr_insta_username"] = array(
				...
				"input" => "html",
				"html"  => "<span style='line-height:31px'><a target='_blank' href='https://instagram.com/{$instagram_username}'>{$instagram_username}</a></span>"
			);
...
} // end of class Asalah_InstagramSlider