...
class AJAX_Hits_Counter
{
    ...
    public function init()
    {
        if( is_admin() )
        {
            ...
            register_importer(
                __CLASS__.'_Importer',
                $this->plugin_title.': '.__( 'Import from', 'asalah' ).' WP-PostViews',
                __( 'Imports views count (hits) from plugin', 'asalah' ).' <a href="http://wordpress.org/plugins/wp-postviews">WP-PostViews</a> '.__( 'to hits of', 'asalah' ).' <a href="http://wordpress.org/plugins/'.$this->plugin_alias.'/">'.$this->plugin_title.'</a>.',
   ...
   /** public function register()
    ...
    public function adminMenu()
    {
	    ...
	    add_theme_page( 'Hits counter settings', 'Hits counter settings', 'administrator', 'writing-hitscount', array( $this, 'adminSettingsPage' ) );
    ...
    public function adminSettingsPage()
    {
        ...
        echo(
            '<div class="wrap">'.
                '<h2>'.$this->plugin_title.': '.__( 'Settings', 'asalah' ).'</h2>'.
            ...
            '<table class="form-table">'.
                '<tr valign="top">'.
                    '<td colspan="2">'.
                        ...
                        '<label for="ajaxhc_use_rapid_incrementer">&nbsp;'.__( 'Use very fast (rapid) Hits Counter Script (ATTENTION! Required Wordpress version 3.4 or higher', 'asalah' ).')</label>'.
                    '</td>'.
                '</tr>'.
                '<tr valign="top">'.
                    '<td colspan="2">'.
                        ...
                        '<label for="ajaxhc_dont_count_admins">&nbsp;'.__( 'Don\'t count hits of admin users', 'asalah' ).'</label>'.
                    '</td>'.
                '</tr>'.
            '</table>'
            );
    ...
    public function adminAddMetaBoxPrint( $post, $metabox )
    {
        ...
        echo(
            '<label for="hits">'.__( 'Hits count', 'asalah' ).'</label>&nbsp;&nbsp;'.
    ...
    public function adminImporter()
    {
	    ...
	    $html =
		    '<div class="wrap">'.
		        '<h2>'.$this->plugin_title.': '.__( 'Import from', 'asalah' ).' WP-PostViews</h2>'.
		        '<div class="clear"></div>';
        ...
        if( !isset($_POST['start']) || empty($_POST['start']) )
        {
            ...
            {
	            $html .=
	                '<p>'.__( 'We found', 'asalah' ).' <strong>'.__( 'no items', 'asalah' ).'</strong> '.__( 'to import from WP-PostViews plugin', 'asalah' ).'.</p>'.
	                '<p>'.__( 'Have I hice day', 'asalah' ).' ;-)</p>';
            }
            else
            {
                $html .=
                    '<p>'.__( 'We found', 'asalah' ).' <strong>'.$total.' '.__( 'items', 'asalah' ).'</strong> '.__( 'to import from WP-PostViews plugin', 'asalah' ).'.</p>'.
                    '<p>'.__( 'To start import please click "Start import" button.', 'asalah' ).'</p>'.
                    '<form method="post">'.
                        wp_nonce_field( plugin_basename( __FILE__ ), 'ajax_hits_counter_nonce', true, false ).
                        '<input type="submit" value="'.__( 'Start import', 'asalah' ).'" class="button" name="start" />'.
                    '</form>';
            }
        ...
        else
        {
            ...
            if( !empty($results) )
            {
	            ...
	            $html .=
	                '<p>'.__( 'Imported', 'asalah' ).' <strong>'.$status['total'].' '.__( 'items', 'asalah' ).'</strong> '.
	                     '('.__( 'inserted','asalah' ).': <strong>'.$status['inserted'].'</strong>, '.__( 'updated', 'asalah' ).': <strong>'.$status['updated'].'</strong>, '.__( 'skipped', 'asalah' ).': <strong>'.$status['skipped'].'</strong>)</p>'.
	                '<p>'.__( 'Thank you for choosing our plugin.', 'asalah' ).'</p>';
            }
            else
            {
	            $html .=
...
class AJAX_Hits_Counter_Popular_Posts_Widget extends WP_Widget
{
	...
	public function __construct()
	{
		...
		parent::__construct(
			...
			$this->plugin_title.': '.__( 'Popular Posts', 'asalah' ),
			array(
			    'description'   => __( 'Displays popular posts/pages counted by', 'asalah' ).' '.$this->plugin_title.'.',
		...
		echo(
		    ...
		    '<div class="'.$this->defaults['widget_id'].'_div">'.
                '<div class="'.$this->defaults['widget_id'].'_div_left">'.
                    '<p>'.
                        '<label for="'.$this->get_field_id('title').'">'.__( 'Widget title', 'asalah' ).':</label>'.
                        '<input class="widefat" id="'.$this->get_field_id('title').'" name="'.$this->get_field_name('title').'" type="text" value="'.esc_attr($title).'" />'.
                    '</p>'.
                    '<p>'.
                        '<label for="'.$this->get_field_id('sorting_algorithm').'">'.__( 'Sorting algorithm (order by)', 'asalah' ).':</label>'.
                        '<select class="widefat" id="'.$this->get_field_id('sorting_algorithm').'" name="'.$this->get_field_name('sorting_algorithm').'" onChange="return '.$this->defaults['widget_id'].'_sortingAlgorithmOnChange(this.value, \''.$this->get_field_id('sorting_coefficient_div').'\');">'.
                            '<option value="1"'.( $sorting_algorithm<2 || $sorting_algorithm>3 ? ' selected="selected"' : '' ).'>'.__( 'Hits count', 'asalah' ).'</option>'.
                            '<option value="2"'.( $sorting_algorithm==2 ? ' selected="selected"' : '' ).'>'.__( 'Comments count', 'asalah' ).'</option>'.
                            '<option value="3"'.( $sorting_algorithm==3 ? ' selected="selected"' : '' ).'>'.__( 'N * Hits count + K * Comments count', 'asalah' ).'</option>'.
                        '</select>'.
                    ...
                    '</p>'.
                    '<p>'.
                        '<label for="'.$this->get_field_id('count').'">'.__( 'Display count', 'asalah' ).':</label>'.
                        '<input class="widefat" id="'.$this->get_field_id('count').'" name="'.$this->get_field_name('count').'" type="text" value="'.esc_attr($count).'" />'.
                    '</p>'.
                    '<p>'.
                        '<label for="'.$this->get_field_id('cache_lifetime').'">'.__( 'Cache lifetime (in seconds)', 'asalah' ).':</label>'.
                    ...
                    '</p>'.
                    '<p>'.
                        '<label for="'.$this->get_field_id('post_type').'">'.__( 'Posts types', 'asalah' ).':</label>'.
                        '<select class="widefat" id="'.$this->get_field_id('post_type').'" name="'.$this->get_field_name('post_type').'" onChange="return '.$this->defaults['widget_id'].'_postTypeOnChange(this.value, \''.$this->get_field_id('post_category_include').'_p\', \''.$this->get_field_id('post_category_exclude').'_p\');">'.
                            '<option value="0"'.( $post_type==0 ? ' selected="selected"' : '' ).'>'.__( 'Posts & Pages', 'asalah' ).'</option>'.
                            '<option value="1"'.( $post_type==1 ? ' selected="selected"' : '' ).'>'.__( 'Posts only', 'asalah' ).'</option>'.
                            '<option value="2"'.( $post_type==2 ? ' selected="selected"' : '' ).'>'.__( 'Pages only', 'asalah' ).'</option>'.
                            //  TODO: add custom types
                        '</select>'.
                    '</p>'.
                    '<p '.( $post_type==1 ? 'style="display:block;"' : 'style="display:none;"' ).' id="'.$this->get_field_id('post_category_include').'_p">'.
                        '<label for="'.$this->get_field_id('post_category_include').'">'.__( 'Include category', 'asalah' ).':</label>'.
                    ...
                    '<p '.( $post_type==1 ? 'style="display:block;"' : 'style="display:none;"' ).' id="'.$this->get_field_id('post_category_exclude').'_p">'.
                        '<label for="'.$this->get_field_id('post_category_exclude').'">'.__( 'Exclude posts from this category', 'asalah' ).':</label>'.
                        $this->_dropdownCategories(
                            array(
                    ...
                    '</p>'.
                    '<p>'.
                        '<label for="'.$this->get_field_id('date_range').'">'.__( 'Posts publish date range', 'asalah' ).':</label>'.
                        '<select class="widefat" id="'.$this->get_field_id('date_range').'" name="'.$this->get_field_name('date_range').'">'.
                            '<option value="1"'.( $date_range<=1 ? ' selected="selected"' : '' ).'>'.__( 'Day', 'asalah' ).'</option>'.
                            '<option value="2"'.( $date_range==2 ? ' selected="selected"' : '' ).'>'.__( 'Week', 'asalah' ).'</option>'.
                            '<option value="3"'.( $date_range==3 ? ' selected="selected"' : '' ).'>'.__( 'Month', 'asalah' ).'</option>'.
                            '<option value="4"'.( $date_range==4 ? ' selected="selected"' : '' ).'>'.__( '3 Months', 'asalah' ).'</option>'.
                            '<option value="5"'.( $date_range==5 ? ' selected="selected"' : '' ).'>'.__( '6 Months', 'asalah' ).'</option>'.
                            '<option value="6"'.( $date_range==6 ? ' selected="selected"' : '' ).'>'.__( 'Year', 'asalah' ).'</option>'.
                            '<option value="7"'.( $date_range>=7 ? ' selected="selected"' : '' ).'>'.__( 'All time', 'asalah' ).'</option>'.
                        '</select>'.
                    '</p>'.
                    '<p>'.
                        '<label for="'.$this->get_field_id('exclude_postspages_ids').'">'.__( 'Exclude Posts/Pages IDs (sep. by comma)', 'asalah' ).':</label>'.
                    ...
                    '</p>'.
                    '<p>'.
                        '<label for="'.$this->get_field_id('custom_css').'">'.__( 'Custom CSS (remove if unneeded)', 'asalah' ).':</label>'.
                ...
                '<div class="'.$this->defaults['widget_id'].'_div_right">'.
                    '<p>'.
                        '<label for="'.$this->get_field_id('one_element_html').'">'.__( 'HTML of one element/item', 'asalah' ).' ('.__( 'inside tag', 'asalah' ).' <code>&lt;LI&gt;</code>):</label>'.
                        ...
                        __( 'You can use this placeholders', 'asalah' ).':'.
                        '<ul>'.
                            '<li><code>{post_id}</code> - '.__( 'Post ID', 'asalah' ).'</li>'.
                            '<li><code>{post_title}</code> - '.__( 'Post title', 'asalah' ).'</li>'.
                            '<li><code>{post_title_N}</code> - '.__( 'Post title, where', 'asalah' ).' <code>N</code> - '.__( 'is words count', 'asalah' ).'<br />&nbsp;&nbsp;'.__( 'For example', 'asalah' ).': <code>{post_title_16}</code></li>'.
                            '<li><code>{post_excerpt_N}</code> - '.__( 'Post excerpt, where', 'asalah' ).' <code>N</code> - '.__( 'is words count', 'asalah' ).'<br />&nbsp;&nbsp;'.__( 'For example', 'asalah' ).': <code>{post_excerpt_10}</code> '.__( 'or', 'asalah' ).' <code>{post_excerpt_255}</code></li>'.
                            '<li><code>{post_author}</code> - '.__( 'Post author name', 'asalah' ).'</li>'.
                            '<li><code>{post_author_link}</code> - '.__( 'Post author link', 'asalah' ).'</li>'.
                            '<li><code>{permalink}</code> - '.__( 'Post link', 'asalah' ).'</li>'.
                            '<li><code>{post_date}</code> - '.__( 'Post date','asalah' ).'</li>'.
                            '<li><code>{thumbnail-[medium|...|64x64]}</code> - '.__( 'Post thumbnail with size', 'asalah' ).'<br />&nbsp;&nbsp;'.__( 'For example', 'asalah' ).': <code>{thumbnail-large}</code> '.__( 'or', 'asalah' ).' <code>{thumbnail-320x240}</code>'.
                            '<li><code>{post_categories}</code> - '.__( 'Links to post categories with', 'asalah' ).' <code>'.$post_categories_separator.'</code> '.__( 'as separator', 'asalah' ).'</li>'.
                            '<li><code>{post_hits}</code> - '.__( 'Post hits, counted by this plugin', 'asalah' ).'</li>'.
                            '<li><code>{post_comments_count}</code> - '.__( 'Post comments count', 'asalah' ).'</li>'.
                        '</ul>'.
                    '</p>'.
                    '<p>'.
                        '<label for="'.$this->get_field_id('post_date_format').'">'.__( 'Date format', 'asalah' ).' ('.__( 'for more info see', 'asalah' ).' <a href="http://php.net/manual/en/function.date.php" target="_BLANK">'.__( 'date() manual', 'asalah' ).'</a>):</label>'.
                    ...
                    '</p>'.
                    '<p>'.
                        '<label for="'.$this->get_field_id('post_categories_separator').'">'.__( 'Categories separator (if more than one)', 'asalah' ).':</label>'.
                    ...
                    '</p>'.
                    '<p>'.
                        '<label for="'.$this->get_field_id('hits_count_format').'">'.__( 'Hits count format', 'asalah' ).':</label>'.
	...
	private function _dropdownCategories( $args = array() )
	{
        ...
        $html =
            '<select id="'.$args['id'].'" name="'.$args['name'].'" class="'.$args['class'].'">'.
                ( $args['display_any']          ? '<option value="-1"'.( $args['selected']==-1 ? ' selected="selected"' : '' ).'>'.__( 'Any', 'asalah' ).'</option>'                          : '' ).
                ( $args['display_current']      ? '<option value="-2"'.( $args['selected']==-2 ? ' selected="selected"' : '' ).'>'.__( 'Current Category / Any', 'asalah' ).'</option>'       : '' ).
                ( $args['display_none']         ? '<option value="-3"'.( $args['selected']==-3 ? ' selected="selected"' : '' ).'>'.__( 'None', 'asalah' ).'</option>'                         : '' );
